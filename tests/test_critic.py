...
from .conftest import GEMMA, QWEN, make_client
...
async def test_critique_happy_path(config):
    ...
    env = await critique(client, config, CritiqueRequest(route="coding", assistant_response="code"))
...
async def test_critique_excludes_generator(config):
    ...
    env = await critique(
        client, config,
        CritiqueRequest(route="coding", assistant_response="x", generator_model=GEMMA),
    )
...
async def test_critique_network_failure_falls_back(config):
    ...
    env = await critique(client, config, CritiqueRequest(route="coding", assistant_response="x"))
...
async def test_critique_unparseable_falls_back(config):
    ...
    env = await critique(client, config, CritiqueRequest(route="general", assistant_response="x"))
    assert env.meta.get("fallback") is True
